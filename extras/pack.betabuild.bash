...
sed -i "s/name=.*/name=zoubworld SAM M0+ Beta Build ${BUILD_NUMBER} (${CURR_TIME})/" platform.txt

cd ..
tar --transform "s|$FOLDERNAME|zoubworld_sam_m0p-${VERSION}-beta-b${BUILD_NUMBER}|g"  --exclude=extras/** --exclude=.git* --exclude=.idea -cjf $FILENAME $FOLDERNAME
cd -
...
sed "s/%%SIZE%%/${SIZE}/" > package_zoubworld_sam_m0p-${VERSION}-beta-b${BUILD_NUMBER}_index.json
